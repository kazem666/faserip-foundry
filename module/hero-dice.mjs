...
export async function promptGeneration(actor) {
  ...
  const form = await DialogV2.wait({
    ...
    content: `<div class="faserip-dialog-scroll"><form class="faserip-feat-dialog"><div class="form-group"><label><input type="checkbox" name="useUpb" ${upbOn ? "checked" : ""} /> Use Ultimate Powers Book (MA3)</label></div><div class="form-group"><label>Advanced Set origin</label><select name="origin">${originOptions}</select></div><div class="form-group"><label><input type="checkbox" name="rollOrigin" ${upbOn ? "" : "checked"} /> Roll Advanced Set origin</label></div><div class="form-group"><label><input type="checkbox" name="secretId" /> Secret identity</label></div></form></div>`,
    buttons: [
