...
import { ensureRollTables } from "./module/roll-tables.mjs";
  ...
  return class FaseripCatalogMenu extends Base {
    ...
    async render(...args) {
      try {
        ...
        try { await ensureRollTables({ notify: true }); } catch (err) { console.warn("FASERIP | roll tables", err); }
...
Hooks.once("init", () => {
  try {
    ...
    game.faserip = {
      ...
      ensureCatalogPacks, fillWorldDefinitions, ensureRollTables, buildCatalogItemData, describeCatalogItem,
...
Hooks.once("ready", () => {
  ...
  Promise.resolve()
    ...
    .then(() => ensureRollTables().catch((err) => console.warn("FASERIP | roll tables", err)))
